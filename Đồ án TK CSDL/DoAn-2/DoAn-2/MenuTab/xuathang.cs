...
namespace DoAn_2.MenuTab
{
    public partial class xuathang : Form
    {
        ...
        {
            connect.Open();
            SqlCommand cmd = new SqlCommand("select count(IDphieu) from Phieu", connect);
            int i = Convert.ToInt32(cmd.ExecuteScalar());
            connect.Close();
            i++;
            txtsohd.Text = i.ToString();
            IDphieu = txtsohd.Text; // luu cho form tt
        }
        public void clearsp()
        {
            txtmasp.Clear();
        ...
        private void btnthem_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("xác nhận người nhận");
            }
        }
        ...
        private void txtmakh_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(txtman.Text))
                {
                    txttenn.Clear();
                }
                else
                {
                    connect.Open();
                    string sqlkh = "select * from KhachHang where SDT=" + int.Parse(txtman.Text);
                    SqlCommand cmd = new SqlCommand(sqlkh, connect);
                    SqlDataReader dr = cmd.ExecuteReader();
                    if (dr.Read())
                    {
                        txttenn.Text = (dr["TenKH"].ToString());
                        //luu tru cho form TT
                        TenKH = txttenn.Text;
                        SDT = txtman.Text;
                    }
