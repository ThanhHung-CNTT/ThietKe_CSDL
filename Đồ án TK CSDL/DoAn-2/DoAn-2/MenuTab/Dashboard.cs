...
namespace DoAn_2.MenuTab
{
    public partial class Dashboard : Form
    {
        ...
        private void doanhsobanhang()
        {
         ...
         //   MessageBox.Show(CurrentMonth);
            try
            {
                ...
                SqlCommand cmdCountTongHDThangNay = new SqlCommand("SELECT count( MONTH([HDtime])) as HDtimene from Phieu where MONTH(HDtime) = '" + CurrentMonth + "'  ", connect);
                SqlCommand cmdCountHDToday = new SqlCommand("select count(IDphieu) from Phieu where cast ([HDtime] as date) = cast(getdate() as date)", connect);
                SqlCommand cmdCountTongTienThangNay = new SqlCommand("select sum(HDtt) from Phieu where MONTH(HDtime) = '" + CurrentMonth + "'  ", connect);
                SqlCommand cmdCountTienToday = new SqlCommand("select sum(HDtt) from Phieu where cast ([HDtime] as date) = cast(getdate() as date)", connect);
                SqlCommand cmdCountDistinctMasp = new SqlCommand("SELECT COUNT(DISTINCT value) FROM Phieu CROSS APPLY STRING_SPLIT(HDmasp, ',') where MONTH(HDtime) = '" + CurrentMonth + "' ", connect);
                SqlCommand cmdCountDistinctMaspToday = new SqlCommand("SELECT COUNT(DISTINCT value) FROM Phieu CROSS APPLY STRING_SPLIT(HDmasp, ',') where cast([HDtime] as date) = cast(getdate() as date) ", connect);
        ...
        private void khachno()
        {
            ...
            {
                connect.Open();
                SqlCommand cmdSumKhachNoThang = new SqlCommand("select sum(HDno) from Phieu where MONTH(HDtime) = '"+ CurrentMonth + "' ", connect);
                SqlCommand cmdKhachNoToday = new SqlCommand("select sum(HDno) from Phieu where cast ([HDtime] as date) = cast(getdate() as date)", connect);
