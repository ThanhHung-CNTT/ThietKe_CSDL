...
namespace DoAn_2.MenuTab
{
    public partial class ChartSLSP : Form
    {
        ...
        private void ChartSLSPdaybydate()
        {
            ...
            SqlDataAdapter adapt = new SqlDataAdapter("SELECT CAST(HDtime AS DATE) AS Ngay, sum(cast(value as int) ) as slsp FROM Phieu CROSS APPLY STRING_SPLIT(HDsl, ',') where MONTH(HDtime) = '" + CurrentMonth + "' group by CAST(HDtime AS DATE)", connect);
